...
contract("Multirewards", function (accounts) {
  ...
  it('send token directy from JFC and call notify rewards with funds transferred to MR contract', async function () {
    await mrContract.setRewardsDistributor(sliceContract.address, jFCContract.address, {from: tokenOwner})
    ...
    await jFCContract.callMRDistribution(mrContract.address, sliceContract.address, jfcTokenBal, {from: tokenOwner})
