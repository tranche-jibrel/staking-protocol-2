...
contract("Multirewards", function (accounts) {
  ...
  it("All other contracts ok", async function () {
    ...
    splitterContract = await TokenSplitter.deployed();
    expect(splitterContract.address).to.be.not.equal(ZERO_ADDRESS);
    expect(splitterContract.address).to.match(/0x[0-9a-fA-F]{40}/);
    console.log(splitterContract.address);

    feeSharingContract = await FeeSharingSystem.deployed();
    expect(feeSharingContract.address).to.be.not.equal(ZERO_ADDRESS);
    expect(feeSharingContract.address).to.match(/0x[0-9a-fA-F]{40}/);
    console.log(feeSharingContract.address);

    distributorContract = await TokenDistributor.deployed();
    expect(distributorContract.address).to.be.not.equal(ZERO_ADDRESS);
    expect(distributorContract.address).to.match(/0x[0-9a-fA-F]{40}/);
    console.log(distributorContract.address);

    tradingRewardsContract = await TradingRewardsDistributor.deployed();
    expect(tradingRewardsContract.address).to.be.not.equal(ZERO_ADDRESS);
    expect(tradingRewardsContract.address).to.match(/0x[0-9a-fA-F]{40}/);
    console.log(tradingRewardsContract.address);
  });
/*
